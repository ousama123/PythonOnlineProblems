...
with open("data4.txt") as f:
    ...
    for line in lines:
        ...
        for i in range(sec2Start, sec2End+1):
         ...
         #check if on list fully contains the other    
        check =  all(item in sections1List for item in sections2List) or all(item in sections2List for item in sections1List)
        if check is True:
            counter = counter + 1
